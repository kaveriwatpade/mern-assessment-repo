...
function App() {
  ...
  const handleSaveResult = () => {
    if (dob && data) {
      // ---------------------------------------------------------
      // OPTION 1: LOCAL STORAGE SAVE (Active - Easy Vercel Deploy)
      // ---------------------------------------------------------
      try {
        const savedResults = JSON.parse(localStorage.getItem('quantumVedicResults') || '[]');
        const newResult = {
          dob,
          data,
        ...
        localStorage.setItem('quantumVedicResults', JSON.stringify(savedResults));
        
        setSavedMessage('Result saved successfully to Local Storage!');
        setTimeout(() => setSavedMessage(''), 3000);
      } catch (err) {
        console.error(err);
        setSavedMessage('Error saving to Local Storage.');
        setTimeout(() => setSavedMessage(''), 3000);
      }

      /* 
      // ---------------------------------------------------------
      // OPTION 2: BACKEND MONGODB SAVE (Commented out for later)
      // ---------------------------------------------------------
      try {
        const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:5000';
        fetch(`${API_URL}/api/results/save`, {
          method: 'POST',
